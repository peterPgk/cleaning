<?php

namespace App\Mail;

use App\MasterSettigns;
use App\Company;
use App\CompanyData;
use App\ServiceCategories;
use Carbon\Carbon;
use Illuminate\Bus\Queueable;
use Illuminate\Mail\Mailable;
use Illuminate\Queue\SerializesModels;
use Illuminate\Contracts\Queue\ShouldQueue;

class ClientBookingEmail extends Mailable
{
    use SerializesModels;

    /**
     * Create a new message instance.
     *
     * @return void
     */

    public $order_data;
    public $total_sum;
    public $booking_fee;
    public $date;
    public $timeslot_from;
    public $timeslot_to;
    public $companyData;
    public $company;
    public $category_name;

    const MAIL_NAME = "ClientBookingEmail";

    public function __construct($order_data, $total_sum, $booking_fee,$company) {
//        throw new \Exception(collect((array)$order_data));
        $this->order_data = $order_data;
        $this->total_sum = $total_sum / 100;
        $this->booking_fee = $booking_fee / 100;
        $this->date = Carbon::parse($order_data->service_date)->format('d/m/Y');

        $aTimelstos = explode('|', $order_data->timeslots);

        $this->timeslot_from = Carbon::parse($aTimelstos[0])->format('H:00');
        $this->timeslot_to = Carbon::parse($aTimelstos[1])->format('H:00');

        $this->companyData = CompanyData::where('company_id', $order_data->company->id)->get()->first();
        $this->company = $company;
        
        $this->category_name = ServiceCategories::where('id', $order_data->services)->pluck('name')->first();
    }

    /**
     * Build the message.
     *
     * @return $this
     */
    public function build()
    {
        return $this->view('email.client-order')->from(env('EMAIL_NOREPLAY'))->subject('Your booking from '.$this->company->name);
    }
}
